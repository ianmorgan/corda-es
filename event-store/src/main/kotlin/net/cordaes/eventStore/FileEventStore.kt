...
class FileEventStore : EventStore {
     ...
     */
    override fun storeEvents(events: List<Event> ) : FileEventStore {
        synchronized(this) {
    ...
    private fun matchesFilter(ev: Event, filter: Filter): Boolean {
        ...
        val matchedTypes = if (filter.types != null) filter.types.contains(ev.type) else true
        val matchedAggregate = if (filter.aggregateId != null) (ev.aggregateId == filter.aggregateId) else true

        return matchedType && matchedTypes && matchedAggregate
    }
}
